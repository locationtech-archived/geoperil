...
class SurferFile:
    ...
    def readSurferBin(self, inp):
        ...
        if buf[0] == b'D' and buf[1] == b'S' and \
                buf[2] == b'B' and buf[3] == b'B':
            self.cols = buf[4]
            self.rows = buf[5]
            self.xmin = buf[6]
            self.xmax = buf[7]
    ...
    def getValueAt(self, row, col, default=None):
        ...
        self.sfile.seek(
            self.headersize + ((row * self.cols + col) * fmtsize)
        )
    ...
    def getRow(self, row):
        ...
        fmtsize = struct.calcsize(fmt)
        self.sfile.seek(
            self.headersize + ((row * self.cols) * fmtsize)
        )
        value = struct.unpack(
            fmt * self.cols, self.sfile.read(fmtsize * self.cols)
        )
        return value
