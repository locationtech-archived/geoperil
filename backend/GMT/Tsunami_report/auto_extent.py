...
def calc_extent_for_w_height(
        wave_height, wave_height_expression, extent, tempdir
):
...
def calc_extent_for_w_time(wave_time, extent, tempdir):
    ...
    subprocess.call(
        ["gdal_polygonize.py", temp_calc_tif, "-f", "GMT", temp_calc_gmt]
    )
...
def tfp_extent(tfp, extent):
    ...
    if tfp_lon_list != []:
        if min(tfp_lon_list) != max(tfp_lon_list):
            tfp_ext[0] = min(tfp_lon_list)
            tfp_ext[1] = max(tfp_lon_list)
            ...
            tfp_ext[0] = tfp_lon_list[0] - 2
            tfp_ext[1] = tfp_lon_list[0] + 2
    ...
    if tfp_lat_list != []:
        if min(tfp_lat_list) != max(tfp_lat_list):
            tfp_ext[2] = min(tfp_lat_list)
            tfp_ext[3] = max(tfp_lat_list)
            ...
            # falls nur ein tfp von welle getroffen
            tfp_ext[2] = tfp_lat_list[0] - 2
            tfp_ext[3] = tfp_lat_list[0] + 2
    ...
    if tfp_lon_list_pos != []:
        if (
            ...
            tfp_ext = tfp_extent_pos

    if tfp_ext[0] or tfp_ext[1] or tfp_ext[2] or tfp_ext[3]:
        extent[0].append(float(tfp_ext[0]))
        extent[1].append(float(tfp_ext[1]))
        extent[2].append(float(tfp_ext[2]))
        extent[3].append(float(tfp_ext[3]))
...
def reformat_coords_if_dateline(west_calc, east_calc, south_calc, north_calc):
    if west_calc > east_calc:
        print(
            "W: %f ; E: %f -> E: %f" % (
                west_calc, east_calc, east_calc + 360
            )
        )
...
def calc_coords(
    ...
    west_calc, east_calc, lon_diff, lat_diff = reformat_coords_if_dateline(
        west_calc, east_calc, south_calc, north_calc
    )
    ...
    # echo $east_calc $north_calc | gmt mapproject \
    # -R${west_calc}/${east_calc}/${south_calc}/${north_calc}
    # -J${crs_system_calc}${map_width_calc}c

    # berechnet breite und hoehe der karte in unit_calc; fuehrt oberen Command
    # aus, entspricht z.B.: gmt mapproject -R-60/5/30/60 -JQ15.8c
    ...
    # entspricht ungefaehr: elif width > height:
        ...
        # Berechnet suedliche Koordinate anhand der eben berechneten
        # Verbesserung
    ...
    # Wandelt Koordinaten die ueber die Datumsgrenze gehen um, da dieses wieder
    # durch die Berechungen umgerechnet wurden
    west_calc, east_calc, lon_diff, lat_diff = reformat_coords_if_dateline(
        west_calc, east_calc, south_calc, north_calc
    )
    ...
    height = float(width_height[1])
    # print (
    #     west_calc, east_calc, south_calc, north_calc, width, height,
    #     lon_diff, lat_diff
    # )
    # print (tempdir)
    return (
