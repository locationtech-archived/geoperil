...
EOF
    ...
    #-D3c/7.65c/6c/0.3ch
    gmt psscale \
        --MAP_ANNOT_OFFSET_PRIMARY=0.06c \
        --MAP_FRAME_PEN=0.02c,black \
...
EOF
    ...
    elif [ ${plot_cfz} == N ] && [ ${plot_tfp} == Y ]; then
        gmt pslegend \
            --FONT_ANNOT_PRIMARY=10p,Helvetica,black \
            -P -R -J \
            ${tfp_cfz_pslegend} \
...
EOF
    ...
    elif [ ${plot_cfz} == Y ] && [ ${plot_tfp} == N ]; then
        gmt pslegend \
            --FONT_ANNOT_PRIMARY=10p,Helvetica,black \
            -P -R -J \
            ${tfp_cfz_pslegend} \
...
if [ ${plot_quake} == Y ]; then
    awk "BEGIN {FS=\",\"}; NR >= 2 {print 0, 0, \$4, \$6, \$7, \$8, \$3, 0, 0;}" ${quake} \
        | gmt psmeca \
...
### disclaimer ###
gmt pslegend \
    --FONT_ANNOT_PRIMARY=6p,Helvetica,85/85/85 \
    -Dx0c/1c/${map_width}c/BL \
    -O -K \
    <<EOF>> ${output}
T ${disclaimer}
EOF
