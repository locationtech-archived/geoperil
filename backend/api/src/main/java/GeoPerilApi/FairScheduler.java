...
public final class FairScheduler implements IScheduler {
    ...
    class Slot {
        ...
        public void schedule() {
            ...
            if (!task.markAsRun()) {
                ...
                System.out.println(
                    "## Task " + task.id + " not in WAIT mode. Skip."
                );
                schedule();
                return;
            }
            ...
            System.out.println(
                "## Worker " + worker + " gets task " + task.id + " assigned"
            );
            worker.putWork(task);
        }
    }
    ...
    private synchronized void waitForWork() {
        /* Check if a pair of task and worker is available that can be
         * scheduled in the next iteration. */
