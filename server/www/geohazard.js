...
function Earthquake(meta) {
  ...
  this.init = function(meta) {
    ...
    /* stores the arrival time upon new isolines should be fetch on the next
     * update */
    this.last_arr = 0;
    /* count pending calls to loadIsos() - needed to avoid race conditions */
    this.pending_isos = 0;

    /* we also want to be notified if the selection status of this event has
     * changed */
    ...
    ajax_mt('webguisrv/getjets', {
      ...
      for (var i = 0; i < result.jets.length; i++) {
        ...
        for (var j = 0; j < jets.length; j++) {
          ...
          pol.setInfoWin(
            new InfoWindow(
              '<span>Wave heights greater than ' + pol.ewh + ' meter.</span>'
            )
          );
...
function Station(meta, eq) {
  ...
  /* most of the time this method will be called asynchronously via
   * setTimeout */
  ...
  this.refresh = function() {
    ...
    if (this.dia == null) {
      ...
      google.visualization.events.addListener(
        this.dia, 'ready', this.ready.bind(this)
      );
    }
...
function MainChartDialog(widget) {
  ...
  this.onFreqInputChange = function() {
    this.setTotalFreq();
  };

  this.onAmplInputChange = function() {
...
function MainChart(widget, width, height) {
  ...
  this.ready = function() {
    ...
    if (this.slider.length == 0) {
      ...
      this.slider.push(
        new Slider(
          false, box.left - 30, box.width + 50,
          ampl,
          box.top + this.div.position().top,
          box.top + this.div.position().top + box.height - 1,
          widget
        )
      );
      this.slider.push(
        new Slider(
          ...
          box.height + 40,
          box.left,
          box.left + this.div.position().left,
          box.left + this.div.position().left + box.width - 1,
          widget
        )
      );
      this.slider.push(
    ...
    for (var j = 0; j < this.slider.length; j++) {
      if (this.values[j] != null)
        this.setSliderValue(j, this.values[j]);
    }
  };
  ...
  this.onControlChange = function(idx) {
    var sliderVal = this.slider[idx].pos.middle;

    if (idx == 0) {
      this.values[idx] = this.dia
        .getChartLayoutInterface()
        .getVAxisValue(sliderVal - this.div.position().top);
    } else {
      this.values[idx] = this.dia
        .getChartLayoutInterface()
        .getHAxisValue(sliderVal - this.div.position().left);
    }
  ...
  this.setSliderValue = function(idx, val) {
    ...
    if (idx == 0) {
      trans = this.dia
        .getChartLayoutInterface()
        .getYLocation(val) + this.div.position().top;
    } else {
      trans = this.dia
        .getChartLayoutInterface()
        .getXLocation(val) + this.div.position().left;
    }
...
function StationView(widget, data) {
  ...
  this.reload = function() {
    ...
    if (!this.widget.is(':visible') || this.data.length() == 0) {
      this.widget.append(
        '<div class="lnkSelect"><a href="javascript:showProp'
        + '(null,\'#propTabStations\')">Select stations</a></div>'
      );
      return;
    }
  ...
  this.deactivateAll = function(excepts) {
    ...
    for (var k = start; k < this.data.length(); k++) {
      this.data.get(k).deactivate();
    }
  };
...
function SingleSymbol(marker, idx, list) {
  ...
  google.maps.event.addListener(
    ...
    function() {
      this.mouseIn = true;
      this.highlight(true);
      this.list.symbolOnMouseEnter(this);
    }.bind(this)
  );
...
function StationSymbols(data, show) {
  ...
  this.symbolOnMouseLeave = function() {
    this.info.close();
    this.removeLine();
  };
    ...
    function success(obj) {
      ...
      for (var i = 0; i < this.scenarios.length; i++) {
        ...
        this.drpNames.find('ul').append(
          '<li class="id_' + id + '"><a href="#">' + name + '</a></li>'
        );
...
function GlobalControl() {
  ...
  this.main = function() {
    ...
    this.layers.insert(
      new PolygonLayer(
        ...
        map,
        function(data) {
          return data.show.waterheights && data.show.waterheights.get();
        },
        function(data) {
          return data.waterheights;
        }
      )
    );
    ...
    if (checkPerm('population')) {
      this.layers.insert(
        new ObjectLayer(
          ...
          map,
          function(data) {
            return data.show.cities.get();
          },
          function(data) {
            return data.objects['city'];
          }, {
            type: 'city',
            newInst: function() {
              return new City();
    ...
    if (checkPerm('flood')) {
      this.layers.insert(new BuildingsLayer('Buildings', map,
        function(data) {
          return data.show.buildings && data.show.buildings.get();
        }));
    }
  ...
  /* Use variable 'dialogs' as a static member without polluting any
   * namespace. */
  // eslint-disable-next-line no-unused-vars
...
function Marker(lat, lon, label, color, map) {
  this.init = function(lat, lon, label, color, map) {
    this.map = null;
    this.link = 'http://chart.apis.google.com/chart?'
  ...
  this.nextEntries = function() {
    ...
    for (var i = this.mid_idx; i < this.data.length(); i++) {
      ...
      if (this.applyFilter(this.data.get(i))) {
        var eqwidget = this.addEntry(
          this.data.get(i),
          this.page * this.half + j + 1
        );
...
function EQWidget(data, marker) {
  ...
  this.init = function(data, marker) {
    ...
    this.div.find('.chk_grid').click(this.onGridChange.bind(this));
    this.div.find('.info a').click(
      (function() {
        this.div.find('.info').hide();
      }).bind(this)
    );
    this.div.find('.lnkEditEvtSet').click(
      this.notifyOn.bind(this, 'clk_edit_set', this.data)
    );
    ...
    if (checkPerm('new_report'))
      this.div.find('.lnkReport').click(
        global.openReportDialog.bind(global, this.data)
      );
    else
      this.div.find('.lnkReport').attr(
        'href', 'webguisrv/generate_report?evtid=' + this.data._id
      );

    this.div.find('.lnkData').click(
      global.openDownloadDialog.bind(global, this.data)
    );
  ...
  this.update = function() {
    ...
    this.div.find('.mag').text(
      prop.magnitude ? prop.magnitude.toFixed(2) : '-'
    );
    this.div.find('.datetime').html(
      datestr + ' &#183; ' + timestr + ' UTC' + ' &#183; ' + this.data._id
    );
    ...
    if ((/^gfz/).test(this.data._id) && this.data.hasCompParams()) {
      ...
      this.div.find('.beach').attr(
        'src', 'http://geofon.gfz-potsdam.de/data/alerts/'
        + yearstr + '/' + data.id + '/bb32.png'
      );
      this.div.find('.geofon').attr(
        'href',
        'http://geofon.gfz-potsdam.de/eqinfo/event.php?id=' + data.id
      );
    } else {
    ...
    if (this.data.abort) {
      ...
      if (process.failed == true) {
        this.div.find('.status').html('Simulation failed').css(
          'color',
          '#d9534f'
        );
        this.div.find('.status').show();
        ...
        this.div.find('.algorithm').html(
          ' &#183; ' + (process.algorithm == 'hysea' ? 'HySEA' : 'EasyWave')
        );
  ...
  this.update = function() {
    ...
    if (this.data.Dir == 'in') {
      if (this.data.ReadTime) {
        var rstr = getDateString(new Date(this.data.ReadTime));
        this.div.find('.stat-read').html('Message read on ' + rstr);
        this.div.find('.stat-read').show();
      }

      if (this.data.MapDisplayTime) {
        var mstr = getDateString(new Date(this.data.MapDisplayTime));
        this.div.find('.stat-disp').html('Map displayed on ' + mstr);
    ...
    if (this.data.Cc) {
      ...
      for (var i = 1; i < this.data.Cc.length; i++) {
        this.div.find('.cc').append(', ' + this.data.Cc[i]);
      }
    } else {
...
/* TODO: EvtSetWidget extends EQWidget at the moment, but invokes init() of
 * Widget! */
...
function EvtSetWidget(data, marker) {
  ...
  this.update = function() {
    ...
    this.div.find('.timestamp').html(
      datestr + ' &#183; ' + timestr + ' UTC' + ' &#183; ' + this.data._id
    );
    this.div.find('.subject').html(this.data.name);
    this.div.find('.mag').text(+prop.magnitude.toFixed(2));
    this.div.find('.latlon').html(
      'Lat ' + prop.latitude + '&deg; &#183; Lon ' + prop.longitude + '&deg;'
    );
    this.div.find('.mag-range').html(
      'Magnitude min: ' + prop.mag_min + ' Mw &#183; step ' + prop.mag_step
      + ' Mw &#183; max ' + prop.mag_max + ' Mw'
    );
    this.div.find('.depth').html(
      'Depth min: ' + prop.depth_min + ' km &#183; step ' + prop.depth_step
      + ' km &#183; max ' + prop.depth_max + ' km'
    );
    this.div.find('.dip').html(
      'Dip min: ' + prop.dip_min + '&deg; &#183; step ' + prop.dip_step
      + '&deg; &#183; max ' + prop.dip_max + '&deg;'
    );
    this.div.find('.strike').html(
      'Strike min: ' + prop.strike_min + '&deg; &#183; step '
      + prop.strike_step + '&deg; &#183; max ' + prop.strike_max + '&deg;'
    );
    this.div.find('.rake').html(
      'Rake min: ' + prop.rake_min + '&deg; &#183; step ' + prop.rake_step
      + '&deg; &#183; max ' + prop.rake_max + '&deg;'
    );
    this.div.find('.dur').html(
      ' &#183; SimDuration ' + this.data.duration + ' min'
    );
    this.div.find('.runtime').html(
      ' &#183; Runtime ' + this.data.calcTime / 1000 + ' sec'
    );
...
function ICallbacks() {
  ...
  this.setCallback = function(action, func) {
    ...
    /* get next id that identifies this callback - can be used to deregister
     * later */
  ...
  this.notifyOn = function(action) {
    ...
    for (var j = 0; j < cnt; j++)
      if (this.callbacks[action][j]) {
        /* TODO: Check this part on production system. */
        // if( vargs.length == 0 ) {
        //   setTimeout(this.callbacks[action][j].bind(this), 0);
        // } else if ( vargs.length == 1 )
        //   setTimeout(this.callbacks[action][j].bind(this, vargs[0]), 0);
        // else if ( vargs.length == 2 )
        //   setTimeout(
        //     this.callbacks[action][j].bind(this, vargs[0], vargs[1]), 0
        //   );
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      for (var i = mlist.length - 1; i >= 0; i--) {
        var entry_m = entries.getOrInsert(new Earthquake(mlist[i]));
        eqlist.insert(entry_m);
      }

      for (var j = ulist.length - 1; j >= 0; j--) {
        var entry_u = entries.getOrInsert(new Earthquake(ulist[j]));
        saved.insert(entry_u);
      }

      for (var k = msglist.length - 1; k >= 0; k--) {
        var entry_l = entries.getOrInsert(new Message(msglist[k]));
        messages.insert(entry_l);

        if (entry_l.parentEvt)
          entry_l.parentEvt = entries.getOrInsert(
            new Earthquake(entry_l.parentEvt)
          );
      }

      for (var l = sets.length - 1; l >= 0; l--) {
        var entry_s = entries.getOrInsert(new EventSet(sets[l]));
        evtsets.insert(entry_s);
      }

      for (var m = floods.length - 1; m >= 0; m--) {
        var entry_f = entries.getOrInsert(new FloodEvent(floods[m]));
        floodsims.insert(entry_f);
      }
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      for (var i = mlist.length - 1; i >= 0; i--) {
        ...
        if (obj['event'] == 'new') {
          entry = entries.getOrInsert(new Earthquake(obj));
          if (!entry.evtset)
            eqlist.insert(entry);
          madd = true;
      ...
      for (var j = ulist.length - 1; j >= 0; j--) {
        ...
        if (obj['event'] == 'new') {
          ...
          entry = entries.getOrInsert(new Message(obj));
          messages.insert(entry);
          msgadd = true;

          if (entry.parentEvt)
            entry.parentEvt = entries.getOrInsert(
              new Earthquake(entry.parentEvt)
            );
        } else if (obj['event'] == 'abort') {
          if (entries.get(id)) {
            entries.get(id).abort = true;
      ...
      for (var k = sets.length - 1; k >= 0; k--) {
        ...
        if (obj['event'] == 'new_evtset') {
          entry = entries.getOrInsert(new EventSet(obj));
      ...
      change = false;
      for (var l = result.floodsim.length - 1; l >= 0; l--) {
        obj = result.floodsim[l];
        if (obj['event'] == 'new') {
          entry = entries.getOrInsert(new FloodEvent(obj.data));
...
function showProp(e, activeTab) {
  ...
  if (curuser.countries) {
    ...
    $('#propStations .check').change(function() {
      $(this).closest('.header').find('.countries input').prop(
        'checked',
        $(this).prop('checked')
      );
      onCountrySelect();
...
function getRegion(country) {
  var regions = {
    IO: [
      'TAN', 'THA', 'IDN', 'IND', 'AUS', 'SRL', 'IRN', 'SEY', 'OMA', 'PAK',
      'KEN', 'MDV', 'MLS', 'MOZ', 'MTS', 'MYA', 'BSH', 'COM', 'MAD', 'SAF',
      'SGP', 'YEM'
    ],
    NEAM: [
      'BEL', 'CYP', 'EGY', 'ESP', 'HEL', 'ISR', 'ITA', 'POR', 'UKR', 'ROM',
      'ANT', 'CBV', 'DEU', 'DMK', 'EIR', 'FRA', 'GRB', 'ISL', 'KNA', 'MLT',
      'MNC', 'MRT', 'NOR', 'SEN', 'SVE', 'IRL', 'PRT', 'TUR', 'GBR', 'GRC',
      'MAR', 'ROU', 'GIB'
    ],
    CARIBE: [
      'BAH', 'BAR', 'CAI', 'COL', 'CRC', 'CUW', 'DOM', 'GRD', 'GUA', 'HAI',
      'JAM', 'MEX', 'MTQ', 'NIC', 'PAN', 'PRC', 'RDO', 'SVG', 'TRT', 'ABR'
    ],
    P: [
      'ANT', 'CAN', 'CHL', 'CKI', 'ECU', 'FIJ', 'FIL', 'HAW', 'HKG', 'JAP',
      'KIR', 'KRS', 'MIC', 'MSI', 'NWZ', 'PAI', 'PER', 'PNG', 'POF', 'RUS',
      'SOI', 'SVD', 'TON', 'TVI', 'USA', 'VAN', 'VNM', 'WSA'
    ],
...
function onCountrySelect() {
  ...
  if (sum > 100) {
    $('#propStations .warn')
      .html(
        "Using more than 100 stations may lead to a noticeable slowdown "
        + "of the entire page!"
      );
  } else {
...
function toggleCloudButton() {
  if (loggedIn) {
    ...
    $('#btnDeselect').data('bs.tooltip').options.title = "Deselect and "
      + "show map only";
  }
}
...
function MailDialog() {
  this.init = function() {
    ...
    this.dialog.content.find('.secMailText .btn').change(
      this.setMsgTexts.bind(this)
    );
  ...
  this.show = function(eq) {
    ...
    var showFax = checkPerm('fax') && checkProp('InterfaxUsername')
      && checkProp('InterfaxPassword');
    var showFtp = checkPerm('ftp') && checkProp('FtpUser')
      && checkProp('FtpHost') && checkProp('FtpPath');

    this.grpCloud.show(checkPerm('intmsg'));
    this.grpMail.show(checkPerm('mail'));
    this.grpFax.show(showFax);
    this.grpFtp.show(showFtp);
    this.grpMailText.show(
      checkPermsAny('intmsg', 'mail') || showFax || showFtp
    );
    this.grpSmsText.show(
      checkPerm('sms') && checkProp('TwilioSID') && checkProp('TwilioToken')
      && checkProp('TwilioFrom')
    );
  ...
  this.setMsgTexts = function() {
    ...
    /* TODO: temporary hack */
    var form = curuser.username == 'test@example.com' ? 'caribe' : null;

    ajax_mt('webguisrv/get_msg_texts', {
      evtid: this.eq._id,
      kind: kind,
      form: form
    }, (function(result) {
...
function BuildingsDialog() {
  ...
  this.updateText = function(buildings) {
    ...
    for (var i = 0; i < buildings.length(); i++) {
      ...
      for (var k = 0; k < tags.length; k++) {
        tag = tags[k][0];
        text += '<td>' + (!b.tags || !b.tags[tag] ? ' - ' : b.tags[tag])
          + '</td>\n';
      }
  ...
  this.show = function(event) {
    ...
    global.loadBuildings(
      event, sw.lng(), sw.lat(), ne.lng(), ne.lat(),
      (function(buildings) {
...
function AdminDialog() {
  this.init = function() {
    ...
    this.drpInst.setToString(function(o) {
      return o.name;
    });
    ...
    this.autoUsers.setToString(function(o) {
      return o.username;
    });
    ...
    for (var perm in this.permInputs)
      grp.content.append(this.permInputs[perm].div);
    ...
    this.tabUser.find('.html-text, .html-check').change(
    ...
    this.autoInst.setToString(function(o) {
      return o.name;
    });
    ...
    for (var inst in this.instInputs)
      grp.content.append(this.instInputs[inst].div);
    grp.expand(true);
  ...
  this.load = function() {
    ...
    ajaxCascade(
      ...
      getAjax('webguisrv/get_stats/', {
        time: time
      }, this.getStats.bind(this, time))
    );
  };
  ...
  this.getStats = function(time, result) {
    ...
    for (var i = 0; i < result.users.length; i++) {
      ...
      for (var j = 0; j < user.logins.length; j++) {
        ...
        var title = 'Logins on ' + date.toISOString().slice(0, 10) + ':  '
          + login.count;
  ...
  this.control = function() {
    ...
    for (var inp in this.inputs)
      modified = modified || (this.inputs[inp].value() != prop[inp]);
  ...
  this.loadUser = function() {
    ...
    /* don't care about undefined values - the HtmlInputField can handle them
     * fine */
    var prop = user.properties ? user.properties : {};
    var perm = user.permissions ? user.permissions : {};
    ...
    for (var inp in this.inputs)
      this.inputs[inp].value(prop[inp]);

    for (var attr in this.permInputs)
      this.permInputs[attr].value(perm[attr]);

    idx = Math.max(this.instlist.getByKey('_id', user.inst).idx, 0);
  ...
  this.saveUser = function(user) {
    ...
    for (var inp in this.inputs)
      user.properties[inp] = this.inputs[inp].value();
  ...
  this.loadInst = function() {
    ...
    for (var i = 0; i < users.length(); i++)
      this.tabInst.find('.users').append(
        '<li>' + users.get(i).username + '</li>'
      );
  };
...
/* TODO: This is just a wrapper to use some new functionality in deprecated
  ...
  this.show = function(evt) {
    ...
    ajax_mt('/webguisrv/gethazardevents', data, (function(result) {
      ...
      for (var i = 0; i < hazards.length; i++) {
        ...
        form.find('.source-provider > img').attr(
          "src", "embed-data/logos/" + hazards[i].provider + ".png"
        );
        form.find('.source-provider > img').attr(
          "alt", "logo-" + hazards[i].provider
        );
        ...
        form.find('.source-coords').html(
          'Lat: ' + hazards[i].y + '&deg;, Lon: ' + hazards[i].x + '&deg;'
        );
        form.find('.source-magnitude').html(
          hazards[i].mag + ' ' + hazards[i].magtype
        );
...
function HtmlDialog(box) {
  this.init = function(box) {
    ...
    this.header = this.div.find(
      '> .modal-dialog > .modal-content > .modal-header'
    );
    this.content = this.div.find(
      '> .modal-dialog > .modal-content > .modal-body'
    );
    this.footer = this.div.find(
...
function TFPLayer(name, map) {
  ...
  this.init = function() {
    google.maps.event.addListener(
      this.map, 'zoom_changed', this.zoomed.bind(this)
    );
  };
  ...
  this.zoomed = function() {
    ...
    /* Re-scaling all markers is clearly a bottleneck and decreases performance
     * significantly.
...
function MarkerLayer(name, map) {
  ...
  this.display = function() {
    this.active_marker.setPosition(
      this.data.prop.latitude, this.data.prop.longitude
    );
  ...
  this.setMap = function(map) {
    ...
    google.maps.event.addListener(
      ...
      (function(e) { // eslint-disable-line no-unused-vars
        /* Evaluate bounds expressions again on zoom change if it is a
         * function. */
...
function CFZResult(meta) {
  ...
  this.toHtml = function() {
    ...
    if (this.eta != -1) {
      txt += '<span>Estimated Arrival Time: ' + min + ':' + sec
        + ' minutes</span><br>';
      txt += '<span>Estimated Wave Height: ' + this.ewh.toFixed(2)
        + ' meters</span><br>';
    } else {
...
function FloodComposeTab(div) {
  ...
  this.init = function() {
    ...
    this.txtName = new HtmlTextGroup('Name:');
    this.txtSourcePoints = new HtmlInputGroup(
      'Sourcepoints:', null, $('.sourcepoints-upload')
    );
    this.txtBreachLocations = new HtmlInputGroup(
      'Breachlocations:', null, $('.breach-upload')
    );
  ...
  this.convertProj = function(op1, op2, reverse) {
    ...
    var utm32n = '+proj=utm +zone=32 +ellps=WGS84 '
  ...
  this.onHydrosChange = function(evt) {
    ...
    reader.onload = (function(e) {
      ...
      for (var l = 3; l < lines.length; l++) {
        ...
        if (/^\s*$/.test(lines[l]))
          continue;
        var values = lines[l].trim().split(/\s+/);
  ...
  this.addPoints = function() {
    ...
    for (var i = 0; i < this.hydroList.length(); i++) {
      ...
      var d = new FloodLocationWidget(point);
...
function IChart() {
  ...
  this.dia = null;
  /* public methods */
  this.setData = function(data) {}; // eslint-disable-line no-unused-vars
  this.draw = function() {};
...
function FloodLocationPoint(data) {
  ...
  this.init = function(data) {
    ...
    this.setCallback('update', (function() {
...
function FloodWidget(data, marker) {
  ...
  this.init = function(data) {
    ...
    this.div.find('.lnkDialog').click(
      global.buildingsDialog.show.bind(global.buildingsDialog, this.data)
    );
  ...
  this.update = function() {
    ...
    this.div.find('.locations').html(
      'Number of locations: ' + this.data.prop.locations
    );
    ...
    if (!this.data.process) {
      ...
      if (this.data.process.progress < 100) {
        this.div.find('.progress-bar').css(
          'width', this.data.process.progress + '%'
        );
        this.div.find('.progress').show();
      } else {
      ...
      this.div.find('.runtime').html(
        'Runtime: ' + this.data.process.calcTime.toFixed(2) + ' s'
      );
    ...
    ajax_mt('webguisrv/getwaterheights', {
      ...
      for (var i = 0; i < result.heights.length; i++) {
        ...
        for (var j = 0; j < heights.length; j++) {
          ...
          pol.setInfoWin(
            new InfoWindow(
              '<span>Water heights greater than ' + pol.height
              + ' meter.</span>'
            )
          );
...
function BuildingsLayer(name, map, fun_enabled) {
  ...
  this.init = function() {
    ...
    /* Reload buildings if bounds of map have changed and remain unchanged for
     * 200 ms. */
    this.map.addListener(
      'bounds_changed',
      DelayedFunction.create(this.getBuildings.bind(this), 200)
    );
  };
...
/* Consolidates all calls to this function until no more invocations arrive for
 * 'ms' milliseconds.
...
function DownloadDialog(data) {
  this.init = function(data) {
    ...
    this.drpProducts.setCallback('change', (function() {
      /* TODO */
      this.drpProducts.selectedItem().file == "custom.png" ?
        this.secOptions.show() : this.secOptions.hide();
    }).bind(this));
  ...
  this.downloadData = function() {
    ...
    var src = 'datasrv/' + evtid + '/' + file + '?date=' + Date.now()
      + '&download=';
    var ret = [];
    for (var key in args)
      ret.push(
        encodeURIComponent('gmt_' + key) + "=" + encodeURIComponent(args[key])
      );
    src += '&' + ret.join('&');

    /* Start download in new iframe to avoid redirects of any kind. The date
     * parameter is necessary to suppress caching. */
    $('<iframe>', {
      src: src
    }).appendTo('.dynamic').hide();
  };
  ...
  this.loadProducts = function() {
    ...
    ajax('datasrv/help', null, (function(result) {
      ...
      ajax('datasrv/gmt_help', null, (function(result) {
        this.params = new Container().setList(result.gmt_params).filter(
          function(o) {
        ...
        for (var i = 0; i < this.params.length(); i++) {
          ...
          if (param.data_type == "Boolean") {
            ...
            param.html.setCallback('change', (function(param) {
              ...
              for (var k = 0; item.change && k < item.change.length; k++) {
                var dep = item.change[k];
                ...
                if (dep_param.data_type == 'R/G/B')
                  dep_param.html.value(
                    'rgb(' + dep.value.split('/').join(',') + ')'
                  );
                if (dep_param.data_type == 'group')
                  dep_param.html.selectByVal('key', dep.value);
              }
              for (var l = 0; item.enable && l < item.enable.length; l++) {
                this.params.findItem('Flag1', item.enable[l]).html.div.show();
              }
              for (var m = 0; item.disable && m < item.disable.length; m++) {
                this.params.findItem('Flag1', item.disable[m]).html.div.hide();
              }
            }).bind(this, param));
          } else {
            param.html = new HtmlTextGroup(param.Flagname);
          ...
          this.categories.findItem('category', param.category)
            .group.content.append(html.div);
        }
        for (var n = 0; n < this.categories.length(); n++) {
          var category = this.categories.get(n);
          category.group.div.removeClass('html-dyngroup')
            .addClass('html-dyngroup2');
          this.secOptions.append(category.group.div);
        }
        /* Fire on change event once for each checkbox to disable all dependent
         * fields. */
        for (var o = 0; o < this.params.length(); o++) {
          param = this.params.get(o);
...
function ComposeForm(div) {
  ...
  this.init = function(div) {
    ...
    /* Create form fields. */
    this.txtName = new HtmlTextGroup('Name:');
    this.txtRoot = new HtmlTextGroup('Root-Id:').setButton('remove')
      .readonly();
    this.txtParent = new HtmlTextGroup('Parent-Id:').setButton('remove')
      .readonly();
    ...
    this.drpAlgo.setToString(function(o) {
      return o.desc;
    });
    this.drpAlgo.setSource(
      new Container().setList(
        [
          {
            name: 'easywave',
            desc: 'EasyWave'
  ...
  this.load = function(data) {
    ...
    this.drpResolution.selectByVal(
      null,
      process ? process.resolution * 60 : null
    );
    this.drpAlgo.selectByVal(
      'name',
      process && process.algorithm ? process.algorithm : null
    );
  ...
  this.check = function() {
    ...
    var hasSlip = this.txtSlip.hasValue() || this.txtLength.hasValue()
      || this.txtWidth.hasValue();
  ...
  this.start = function() {
    ...
    var data = {
      ...
      date: this.txtDate.value() ?
        this.txtDate.div.data('dateObj').toISOString() : null,
...
function MapObject() {
  ICallbacks.call(this);
  this.create = function() {};
  this.show = function(map) {}; // eslint-disable-line no-unused-vars
  this.hide = function(map) {}; // eslint-disable-line no-unused-vars
}
...
function City() {
  ...
  this.create = function(obj) {
    this.point = new Rectangle(obj.lat, obj.lon, map)
      .setInfoWin(new InfoWindow());
    return this;
  };
  ...
  this.show = function(map, obj, data) {
    var html = '<b>' + obj.name + ' - ' + obj.adm0 + '</b><br>'
      + '<span>Population: ' + obj.popmax.toLocaleString("en-US")
      + '</span><br>' + '<span>Estimated Wave Height: ' + data.ewh + '</span>';
...
function ReportCustomDialog() {
  ...
  this.draw = function(pages) {
    /* Attention: $.empty() also removes event listeners which will destroy
     * the group elements. */
    /* Thus, call detach() on each child element instead, which will retain
     * all listeners. */
      ...
      if (i > 0) {
        span.append(
          $('<a><span class="glyphicon glyphicon-arrow-up"></span></a>')
            .click(func.bind(this, pages, i, i - 1))
        );
      }
      if (i < pages.length() - 1) {
        span.append(
