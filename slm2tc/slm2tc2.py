...
def limitruntime(args):
    func, args, limit = args
    t = threading.Thread(target=func, args=(args,))
    t.daemon = True
    t.start()
    t.join(limit)
    if t.is_alive():
        print("Thread %s won't stop." % str(t.ident))
...
if cmd=="retrievedata":
    ...
    for s in config["stations"].values():
        args.append((getdataforstation,(config,s),TIMEOUT*5))
    tp = ThreadPool(config["threads"])
    tp.map(limitruntime,args)
