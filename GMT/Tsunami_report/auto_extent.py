...
def get_maxmin_wave_height(wave_height):
    wave_height_info = subprocess.Popen(
        ['gdalinfo', wave_height, '-mm'],
        stdout=subprocess.PIPE
    ).stdout.read().decode("utf-8")
    wave_height_max = re.findall(
        r"Min/Max=(-?\d+.\d+),(-?\d+.\d+)",
        wave_height_info
    )
...
# Falls keine/oder nicht vollstaendige Ausdehnung eingegeben wird, wird die
# Ausdehnung automatisch anhand des Tsunami-GRIDs und w_exp berechnet
def calc_extent_for_w_height(
        wave_height,
    ...
    # if (west is None) or (east is None) or (north is None)
    # or (south is None):
    ...
    extent_info = subprocess.Popen(
        ['ogrinfo', '-al', '-so', temp_extent_file1],
        stdout=subprocess.PIPE
    ).stdout.read().decode("utf-8")
    ...
    extent_w_height = re.findall(r"\((-?\d+.\d+), (-?\d+.\d+)\)", extent_info)
    '''
...
#########################
# Falls keine/oder nicht vollstaendige Ausdehnung eingegeben wird, wird die
# Ausdehnung automatisch anhand des Tsunami-Traveltime-GRIDs berechnet
def calc_extent_for_w_time(wave_time, extent, tempdir):
    ...
    # if (west is None) or (east is None) or (north is None) or
    # (south is None):
    # entnimmt dem traveltime-GRID den hoechsten Z-Wert
    wave_time_info = subprocess.Popen(
        ['gdalinfo', wave_time, '-mm'],
        stdout=subprocess.PIPE
    ).stdout.read().decode("utf-8")
    wave_time_max = re.findall(
        r"Min/Max=(-?\d+.\d+),(-?\d+.\d+)",
        wave_time_info
    )
    ...
    # berechnet automatisch maximalste Ausdehnung des Traveltime-GRIDs fuer
    # alle Z-Werte unter wave_time_max_extent
    subprocess.call([
        'gdal_calc.py',
        '-A',
        wave_time,
        '--outfile=%s' % temp_calc_tif,
        '--calc=logical_and(A>=0.0000001, A<=%s)' % (wave_time_max_extent)
    ])
    subprocess.call([
        'gdal_polygonize.py',
        temp_calc_tif,
    ...
    gmt_file.close()
    # liest die Koordinaten fuer die Ausdehnung aus der zweiten Zeile der
    # GMT-File
    extent_w_time = re.findall(r"(-?\d+.\d+)", extent_line)
...
def cfz_extent(cfz, extent):
    ...
    # liest die Koordinaten fuer die Ausdehnung aus der zweiten Zeile der
    # GMT-File
    extent_cfz = re.findall(r"(-?\d+.\d+)", cfz_extent_line)
    '''
...
def tfp_extent(tfp, extent):
    ...
    tfp_lon_list = []
    tfp_lat_list = []
    tfp_extent_list = [None, None, None, None]
    ...
    if tfp_lon_list != []:
        if min(tfp_lon_list) != max(tfp_lon_list):
            tfp_extent_list[0] = min(tfp_lon_list)
            tfp_extent_list[1] = max(tfp_lon_list)
            ...
            tfp_extent_list[0] = tfp_lon_list[0] - 2
            tfp_extent_list[1] = tfp_lon_list[0] + 2
    ...
    if tfp_lat_list != []:
        if min(tfp_lat_list) != max(tfp_lat_list):
            tfp_extent_list[2] = min(tfp_lat_list)
            tfp_extent_list[3] = max(tfp_lat_list)
            ...
            # falls nur ein tfp von welle getroffen
            tfp_extent_list[2] = tfp_lat_list[0] - 2
            tfp_extent_list[3] = tfp_lat_list[0] + 2
    ...
    if tfp_lon_list_pos != []:
        if (
                (tfp_extent_pos[1] - tfp_extent_pos[0]) <
                (tfp_extent_list[1] - tfp_extent_list[0])
        ):
            tfp_extent_list = tfp_extent_pos
    if (
            tfp_extent_list[0] or
            tfp_extent_list[1] or
            tfp_extent_list[2] or
            tfp_extent_list[3]
    ):
        extent[0].append(float(tfp_extent_list[0]))
        extent[1].append(float(tfp_extent_list[1]))
        extent[2].append(float(tfp_extent_list[2]))
        extent[3].append(float(tfp_extent_list[3]))
    return extent
...
# combine calc extent #
#######################
def best_auto_extent_for_input(
        west,
        east,
        south,
        north,
    ...
    if (west is None) or (east is None) or (north is None) or (south is None):
        ...
        if not wave_height == '':
            ...
            if wave_height_expression < wave_height_max:
                extent = calc_extent_for_w_height(
                    wave_height,
                    wave_height_expression,
                    extent,
                    tempdir
...
###############################################################################
# Funktionen zur automatischen Berechnung der Groesse des Kartenrahmes anhand
# Eingabe-Koords sowie x_ratio und y_ratio#
###############################################################################

# falls der Kartenausschnit ueber die Datumsgrenze (180°) geht muessen die
...
# Berechnet automatisch den Kartenrahmen nach Eingabe der Koordinaten und des
...
def calc_coords(
    ...
    west_calc, east_calc, lon_diff, lat_diff = reformat_coords_if_dateline(
        west_calc,
        east_calc,
        south_calc,
        north_calc
    ...
    # echo $east_calc $north_calc | \
    # gmt mapproject -R${west_calc}/${east_calc}/${south_calc}/${north_calc} \
    # -J${crs_system_calc}${map_width_calc}c

    # berechnet breite und hoehe der karte in unit_calc;
    # fuehrt oberen Command aus
    # entspricht z.B.: gmt mapproject -R-60/5/30/60 -JQ15.8c
    calc_width_height_command = ['gmt', 'mapproject', extent, projection]
    mapproject = subprocess.Popen(
        calc_width_height_command,
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE, cwd=tempdir
    )
    # bytes("%f %f\n" % (east_calc,north_calc),"ascii") =
    #     echo east_calc north_calc
    out, err = mapproject.communicate(
        bytes("%f %f\n" % (east_calc, north_calc), "ascii")
    )
    print(out, err)
    ...
    if (height / width) > (y_ratio_calc / x_ratio_calc):
        ...
        # Berechnet oestliche Koordinate anhand der eben berechneten
        # Verbesserung
        calc_west_calc = subprocess.Popen(
            calc_coords_command,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            cwd=tempdir
        )
        west_calc = calc_west_calc.communicate(
            bytes("%f %f\n" % (west_calc_unit_calc, height), "ascii")
        )[0].decode().split()
        west_calc = round(float(west_calc[0]), 2)

        # Berechnet westliche Koordinate anhand der eben berechneten
        # Verbesserung
        calc_east_calc = subprocess.Popen(
            calc_coords_command,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            cwd=tempdir
        )
        east_calc = calc_east_calc.communicate(
            bytes("%f %f\n" % (east_calc_unit_calc, height), "ascii")
        )[0].decode().split()
    ...
    # Berechnet suedliche Koordinate anhand der eben berechneten Verbesserung
        calc_south_calc = subprocess.Popen(
            calc_coords_command,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            cwd=tempdir
        )
        south_calc = calc_south_calc.communicate(
            bytes("%f %f\n" % (width, south_calc_unit_calc), "ascii")
        )[0].decode().split()
        south_calc = round(float(south_calc[1]), 2)

        # Berechnet noerdliche Koordinate anhand der eben berechneten
        # Verbesserung
        calc_north_calc = subprocess.Popen(
            calc_coords_command,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            cwd=tempdir
        )
        north_calc = calc_north_calc.communicate(
            bytes("%f %f\n" % (width, north_calc_unit_calc), "ascii")
        )[0].decode().split()
        north_calc = round(float(north_calc[1]), 2)
    # else:
    #     Koordinaten sind so perfekt, dass sie genau das Seitenverhaeltnis
    #     abdecken

    # Wandelt Koordinaten die ueber die Datumsgrenze gehen um, da dieses
    # wieder durch die Berechungen umgerechnet wurden
    west_calc, east_calc, lon_diff, lat_diff = reformat_coords_if_dateline(
        west_calc,
        east_calc,
        south_calc,
    ...
    extent = '-R%s/%s/%s/%s' % (west_calc, east_calc, south_calc, north_calc)
    mapproject = subprocess.Popen(
        ['gmt', 'mapproject', extent, projection],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        cwd=tempdir
    )
    width_height = mapproject.communicate(
        bytes("%f %f\n" % (east_calc, north_calc), "ascii")
    )[0].decode().split()

    width = float(width_height[0])
    height = float(width_height[1])
    # print (west_calc, east_calc, south_calc, north_calc, width, height,
    #       lon_diff, lat_diff)
    # print (tempdir)
    return (
        west_calc,
